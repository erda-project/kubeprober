...
build-cli: generate fmt vet ## Build manager binary.
	go build -o dist/${GOOS}_${APP} ./cli/${APP}/${APP}.go
...
deploy-build: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	$(KUSTOMIZE) build config/${APP} > deployment/${APP}.yaml
...
# go-get-tool installs any package $2 and places the binary at $1.
