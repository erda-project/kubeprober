...
func makeRequestToDeploymentCheckService(ctx context.Context, client *kubernetes.Clientset) error {
	// service ip
	hostname := getServiceClusterIP(ctx, client)
	...
	go func() {
		...
		case result := <-getRequestBackoff(hostname):
			...
			requestChan <- nil
		case <-timeoutChan:
			err := fmt.Errorf("backoff loop for a 200 response took too long and timed out")
			requestChan <- err
		case <-ctx.Done():
			err := fmt.Errorf("context expired while waiting for a status: %v form hostname: %s", http.StatusOK, hostname)
			requestChan <- err
		}
