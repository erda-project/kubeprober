...
function sential_detect() {
    ...
    if [ "$REPLICAS" != "$READY" ]; then
      report-status --name=redis_sential_check --status=error --message="redis sentail ready pod not statisfied expect"
      return 1
    else
      ...
      do
        if ! nc -z -w 1 "$i" "$S_PORT">/dev/null 2>/dev/null; then
          report-status --name=redis_sential_check --status=error  --message="redis_sential_check error redis-sentinal:$i $S_PORT connect failed"
          return 1
        fi
      done
      report-status --name=redis_sential_check --status=ok
    fi
}
...
function redis_detect() {
    ...
    if [ "$REPLICAS" != "$READY" ]; then
        report-status --name=redis_check --status=error --message="redis ready pod not statisfied expect"
        return 1
    else
      MASTER_INFO=$(echo info | nc "$S_IP" "$S_PORT"  2>/dev/null| grep master0)
      if [ $? != 0 ];then
          report-status --name=redis_check --status=error --message="get redis info failed"
          return 1
      else
          ...
          do
            if ! nc -z -w 1 "$i" "$R_PORT">/dev/null 2>/dev/null; then
              report-status --name=redis_check --status=error --message="redis:$i $R_PORT connect failed"
              return 1
            fi
          done

          if ! ((printf "AUTH $R_PASSWORD\r\n";  echo set probe_test_key v1) | nc $R_IP $R_PORT) | tail -n 1 | grep OK >/dev/null 2>/dev/null ; then
            report-status --name=redis_check  --status=error --message="redis:$R_IP write key error"
            return 1
          fi

          if ! ((printf "AUTH $R_PASSWORD\r\n";  echo del probe_test_key) | nc $R_IP $R_PORT) | tail -n 1 | grep ":1"  >/dev/null 2>/dev/null; then
            report-status --name=redis_check --status=error --message="redis:$R_IP delete key error"
            return 1
          fi
          ...
          do
            if ! ((printf "AUTH $R_PASSWORD\r\n";  echo info replication) | nc $i  "$R_PORT" 2>/dev/null) | grep "master_link_status:up">/dev/null 2>/dev/null; then
              report-status --name=redis_check --status=error  --message="redice slave:$i psync failed"
              return 1
            fi
          done
      fi
      report-status --name=redis_check --status=ok --message="-"
    fi
}
